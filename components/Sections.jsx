...
const Certs = () => {
  ...
  const items = (!loading && badges)
    ? badges.map((b, i) => ({
        key: b.id || i,
        name: b.badge_template?.name || b.name || '',
        issuer: b.badge_template?.issuer?.name || '',
        year: b.issued_at_date ? b.issued_at_date.slice(0, 4) : (b.issued_at ? b.issued_at.slice(0, 4) : ''),
          ...
          {loading ? (
            ...
            [0, 1, 2].map(i => (
              <div className="cert" key={i} style={{ opacity: 0.4 }}>
                <div className="badge-ring" style={{ background: 'var(--border)' }}><div></div></div>
                <div>
                  <div className="issuer" style={{ background: 'var(--border)', width: '60%', height: 10, borderRadius: 4 }}></div>
                  <div className="name" style={{ background: 'var(--border)', width: '80%', height: 14, borderRadius: 4, marginTop: 6 }}></div>
                </div>
              </div>
            ))
          ) : items ? (
            items.map(c => (
              <div className="cert" key={c.key}>
                ...
                <img
                  src={c.image}
                  alt={c.name}
                  style={{ width: 44, height: 44, borderRadius: '50%', objectFit: 'contain', background: 'var(--card-2)' }}
                />
                <div>
        ...
        <div className="proj-list">
          {d.projects.map(p => (
            <div className="proj" key={p.idx}>
              ...
              <div className="arrow"><Icon name="arrowUp" size={14} /></div>
            </div>
          ))}
        </div>
      </div>
          ...
          {d.experience.map(e => (
            <div className="exp" key={e.title + e.date}>
              <div className="exp-head">
                <div className="exp-title">{e.title} · <span className="co">{e.company}</span></div>
    ...
    <section id="contact">
      <div className="wrap">
        ...
        <div className="contact-card">
          <div>
            <h2>Got a platform puzzle<br />or just wanna <em>talk anime?</em></h2>
            <p className="sub">I'm always up for a chat — whether it's a consulting conversation, a collab, or deciding once and for all whether Gear 5 beats Ultra Instinct.</p>
            <div style={{ position: 'relative' }}>
              <span className="mono" style={{ fontSize: 11, color: 'var(--text-mute)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>RESPONSE TIME · &lt; 24H</span>
            </div>
          </div>
          <div className="contact-list">
            {d.contact.map(c => (
              <a className="contact-row" key={c.lbl} href={c.href} target={c.href.startsWith('http') ? '_blank' : undefined} rel="noreferrer">
                <div className="left">
                  <div className="icon"><Icon name={c.icon} size={16} /></div>
                  <div>
