...
describe('Certs component', () => {
  ...
  it('falls back to data.js certs when fetch fails', async () => {
    ...
    render(<Certs />);
    await waitFor(() => {
      expect(screen.getByText(window.PORTFOLIO.certs[0].name)).toBeInTheDocument();
    });
  });
...
describe('Projects component', () => {
  it('renders all projects from PORTFOLIO', () => {
    render(<Projects />);
    window.PORTFOLIO.projects.forEach((p) => {
...
describe('Experience component', () => {
  it('renders all experience titles', () => {
    render(<Experience />);
    window.PORTFOLIO.experience.forEach((e) => {
  ...
  it('renders tag chips', () => {
    render(<Experience />);
    window.PORTFOLIO.experience[0].tags.forEach((t) => {
...
describe('Contact component', () => {
  it('renders all contact entry labels and values', () => {
    render(<Contact />);
    window.PORTFOLIO.contact.forEach((c) => {
  ...
  it('renders email link with correct href', () => {
    render(<Contact />);
    const entry = window.PORTFOLIO.contact.find((c) => c.lbl === 'Email');
  ...
  it('renders external links with target=_blank and rel=noreferrer', () => {
    render(<Contact />);
    const entry = window.PORTFOLIO.contact.find((c) => c.lbl === 'GitHub');
...
describe('Foot component', () => {
  it('renders the verse from PORTFOLIO', () => {
    render(<Foot />);
    expect(screen.getByText(window.PORTFOLIO.verse)).toBeInTheDocument();
  });
